  ...
  paths: {
              ...
              'application/json': {
                ...
                example: {
                  ...
                  components: {
                    ...
                    strix_cli: { status: 'ok', path: '/usr/local/bin/strix', message: 'strix found' },
                    storage: { status: 'ok', runs_dir: '/app/strix_runs', runs_dir_exists: true, total_scans: 5, running_scans: 1 },
                  },
                },
              },
            },
    ...
    '/scans': {
      get: {
        ...
        description: 'Returns a list of all scans sorted by start time (newest first), each with status and vulnerability count.',
          ...
          '200': {
            ...
            content: {
              'application/json': {
                ...
                example: { scanId: 'uuid-here', status: 'running', mode: 'live' },
              },
            },
          },
          '400': {
    ...
    '/scans/{id}': {
      get: {
        ...
        description: 'Returns the full scan metadata including status, model used, timestamps, and all discovered vulnerabilities.',
        operationId: 'getScan',
      ...
      delete: {
        ...
        description: 'Sends SIGTERM to the running scan process and marks it as stopped.',
    ...
    schemas: {
      ...
      CreateScanRequest: {
        ...
        properties: {
          target: {
            type: 'string',
            description: 'Target URL, GitHub repository URL, or local directory path',
          ...
          apiKey: {
            type: 'string',
            description: 'API key for the selected LLM provider (never stored on disk)',
            example: 'sk-...',
          },
          scanMode: {
            type: 'string',
          ...
          instruction: {
            ...
            example: 'Focus on authentication bypasses and IDOR. Use credentials: admin/password123',
          },
        },
      },
      CreateScanResponse: {
        type: 'object',
        properties: {
          scanId: { type: 'string', format: 'uuid', description: 'Unique scan identifier' },
          status: { type: 'string', enum: ['running'] },
          mode: { type: 'string', enum: ['live', 'demo'], description: 'live = real strix process; demo = mock data (strix not installed)' },
        },
      },
      ScanSummary: {
        type: 'object',
        properties: {
          ...
          vulnCount: { type: 'integer', description: 'Number of discovered vulnerabilities' },
        },
      },
      ScanDetail: {
        allOf: [
      ...
      Vulnerability: {
        ...
        properties: {
          ...
          poc: { type: 'string', description: 'Proof of Concept — reproduction steps or payload' },
          cvss: { type: 'number', format: 'float', minimum: 0, maximum: 10, example: 9.8 },
