...
export async function POST(req: NextRequest) {
  ...
  proc.stdout?.on('data', (chunk: Buffer) => {
    ...
    // Try to parse vulnerability lines
    parseVulnFromLog(chunk.toString(), vulnFile);
  });
...
// Parse vulnerability JSON blocks embedded in log output
function parseVulnFromLog(text: string, vulnFile: string) {
...
// Demo/mock mode when strix CLI is not installed
function runMockScan(
  ...
  const interval = setInterval(() => {
    if (i < logs.length) {
      ...
      // Add vulns at specific log steps
      if (i === 7) { const v = [mockVulns[0]]; fs.writeFileSync(vulnFile, JSON.stringify(v, null, 2)); }
      if (i === 9) { const v = mockVulns.slice(0, 2); fs.writeFileSync(vulnFile, JSON.stringify(v, null, 2)); }
      if (i === 11) { const v = mockVulns.slice(0, 3); fs.writeFileSync(vulnFile, JSON.stringify(v, null, 2)); }
      if (i === 13) { const v = mockVulns.slice(0, 4); fs.writeFileSync(vulnFile, JSON.stringify(v, null, 2)); }
      if (i === 15) { fs.writeFileSync(vulnFile, JSON.stringify(mockVulns, null, 2)); }
      i++;
    } else {
