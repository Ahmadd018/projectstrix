...
function getStrixCommand(): string {
  ...
  log.warn('STRIX_CMD', 'strix not found in any candidate path, falling back to PATH lookup', { candidates });
  return 'strix';
}
...
export async function GET() {
  ...
  try {
        ...
        try {
          ...
          if (fs.existsSync(vulnFile)) {
            try {
              ...
              log.warn('GET /api/scans', `Failed to parse vulnerabilities.json for ${e.name}`, { err: String(e2) });
            }
          }
          log.debug('GET /api/scans', `Scan ${e.name}: status=${data.status} vulns=${vulnCount}`);
          return { ...data, vulnCount };
        } catch (e2) {
          log.error('GET /api/scans', `Failed to parse run.json for ${e.name}`, e2);
          return null;
        }
      })
      .filter(Boolean)
    ...
    return NextResponse.json({ scans: [], error: 'Failed to list scans' }, { status: 200 });
...
export async function POST(req: NextRequest) {
  ...
  log.info('POST /api/scans', `Spawning strix process`, { cmd: strixCmd, args });
  ...
  try {
    ...
    log.error('POST /api/scans', `Failed to spawn strix (ENOENT likely — not installed)`, err);
  ...
  proc.stdout?.on('data', (chunk: Buffer) => {
    ...
    log.debug('PROC_STDOUT', `[${scanId.slice(0, 8)}] ${text.slice(0, 200).trim()}`);
  ...
  proc.stderr?.on('data', (chunk: Buffer) => {
    ...
    log.warn('PROC_STDERR', `[${scanId.slice(0, 8)}] ${text.slice(0, 500).trim()}`);
  });
  ...
  proc.on('close', (code: number | null) => {
    ...
    updated.status = code === 0 ? 'completed' : code === null ? 'stopped' : 'failed';
    updated.finishedAt = new Date().toISOString();
    updated.exitCode = code;
    fs.writeFileSync(runFile, JSON.stringify(updated, null, 2));
    log.info('PROC_CLOSE', `Scan ${scanId.slice(0, 8)} finished`, { exitCode: code, status: updated.status });
  });

  proc.on('error', (err: Error) => {
    log.error('PROC_ERROR', `Process error for scan ${scanId.slice(0, 8)}`, err);
...
function parseVulnFromLog(text: string, vulnFile: string, scanId: string) {
  ...
  while ((match = vulnPattern.exec(text)) !== null) {
    try {
      ...
      let existing: any[] = [];
      try { existing = JSON.parse(fs.readFileSync(vulnFile, 'utf-8')); } catch {}
      if (!existing.find((v: any) => v.id === vuln.id)) {
        existing.push(vuln);
        fs.writeFileSync(vulnFile, JSON.stringify(existing, null, 2));
        log.info('VULN_PARSER', `New vulnerability found for scan ${scanId.slice(0, 8)}`, {
          id: vuln.id,
          title: vuln.title,
      ...
      log.warn('VULN_PARSER', 'Failed to parse vulnerability from log', { err: String(e) });
    }
  }
}
...
function runMockScan(
  ...
  log.info('MOCK_SCAN', `Starting mock scan for ${target}`, { scanId: scanId.slice(0, 8) });

  const mockVulns = [
    { id: 'v1', title: 'SQL Injection in /api/login', severity: 'critical', endpoint: '/api/login', method: 'POST', description: 'The login endpoint is vulnerable to SQL injection via the username parameter.', poc: "username=admin' OR 1=1--&password=x", cvss: 9.8, remediation: 'Use parameterized queries or prepared statements.' },
    { id: 'v2', title: 'Reflected XSS in Search', severity: 'high', endpoint: '/search', method: 'GET', description: 'The q parameter is reflected without sanitization, allowing script injection.', poc: '/search?q=<script>alert(1)</script>', cvss: 7.4, remediation: 'Encode all user-supplied data before rendering in HTML context.' },
    { id: 'v3', title: 'IDOR on /api/users/{id}', severity: 'high', endpoint: '/api/users/{id}', method: 'GET', description: 'Any authenticated user can read any other user profile by changing the ID.', poc: 'GET /api/users/1 with any valid JWT', cvss: 7.1, remediation: 'Enforce ownership checks server-side for all resource access.' },
    { id: 'v4', title: 'Missing Rate Limiting on /api/auth', severity: 'medium', endpoint: '/api/auth', method: 'POST', description: 'No rate limiting allows brute-force credential attacks.', poc: 'Send 1000 requests/min with no throttling', cvss: 5.3, remediation: 'Implement rate limiting and account lockout after failed attempts.' },
    { id: 'v5', title: 'Exposed Debug Endpoint', severity: 'low', endpoint: '/debug/config', method: 'GET', description: 'Configuration details including environment variables are exposed.', poc: 'GET /debug/config returns full server config', cvss: 3.1, remediation: 'Disable or restrict debug endpoints in production.' },
  ];
  ...
  const logs = [
    ...
    `[${new Date().toISOString()}] 🕷️  Discovered 24 endpoints`,
    ...
    `[${new Date().toISOString()}] ℹ️  MEDIUM: No rate limiting on /api/auth`,
    `[${new Date().toISOString()}] ⚡ Checking for exposed endpoints...`,
    `[${new Date().toISOString()}] ℹ️  LOW: Debug endpoint exposed at /debug/config`,
  ...
  const interval = setInterval(() => {
    if (i < logs.length) {
      ...
      if (i === 7) { const v = [mockVulns[0]]; fs.writeFileSync(vulnFile, JSON.stringify(v, null, 2)); log.info('MOCK_SCAN', 'Added vuln: SQL Injection'); }
      if (i === 9) { const v = mockVulns.slice(0, 2); fs.writeFileSync(vulnFile, JSON.stringify(v, null, 2)); log.info('MOCK_SCAN', 'Added vuln: Reflected XSS'); }
      if (i === 11) { const v = mockVulns.slice(0, 3); fs.writeFileSync(vulnFile, JSON.stringify(v, null, 2)); log.info('MOCK_SCAN', 'Added vuln: IDOR'); }
      if (i === 13) { const v = mockVulns.slice(0, 4); fs.writeFileSync(vulnFile, JSON.stringify(v, null, 2)); log.info('MOCK_SCAN', 'Added vuln: Missing Rate Limit'); }
      if (i === 15) { fs.writeFileSync(vulnFile, JSON.stringify(mockVulns, null, 2)); log.info('MOCK_SCAN', 'Added all 5 vulnerabilities'); }
      i++;
    } else {
      ...
      updated.exitCode = 0;
      fs.writeFileSync(runFile, JSON.stringify(updated, null, 2));
      log.info('MOCK_SCAN', `Mock scan ${scanId.slice(0, 8)} completed`, { totalVulns: mockVulns.length });
    }
  }, 1500);
}
