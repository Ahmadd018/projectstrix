  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      if (fs.existsSync(logFile)) {
        ...
        log.debug(`SSE /api/scans/${id}/stream`, `Replaying ${lines.length} existing log lines`);
        for (const line of lines) {
          controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'log', line })}\n\n`));
        }
      } else {
        log.debug(`SSE /api/scans/${id}/stream`, 'No log.txt yet — scan may not have started output');
      }
      ...
      if (fs.existsSync(runFile)) {
        try {
          ...
          if (run.status !== 'running') {
            log.info(`SSE /api/scans/${id}/stream`, `Scan already finished (status=${run.status}), closing SSE immediately`);
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'status', status: run.status })}\n\n`));
            controller.close();
            return;
          }
        } catch (e) {
          log.error(`SSE /api/scans/${id}/stream`, 'Failed to read run.json', e);
        }
      }
      ...
      log.debug(`SSE /api/scans/${id}/stream`, `Starting file watch (initial log size: ${lastSize} bytes)`);
      ...
      const watchInterval = setInterval(() => {
        ...
        if (tickCount % 60 === 0) {
          log.debug(`SSE /api/scans/${id}/stream`, `Heartbeat tick=${tickCount}, logSize=${lastSize}, vulns=${lastVulnCount}`);
        }
        ...
        if (fs.existsSync(logFile)) {
          try {
            ...
            if (stat.size > lastSize) {
              ...
              log.debug(`SSE /api/scans/${id}/stream`, `New log data: ${lines.length} lines (+${stat.size - lastSize} bytes)`);
              lastSize = stat.size;
              for (const line of lines) {
                try {
                  controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'log', line })}\n\n`));
                } catch (e) {
                  log.error(`SSE /api/scans/${id}/stream`, 'Failed to enqueue log line', e);
                }
              }
            }
          } catch (e) {
            log.error(`SSE /api/scans/${id}/stream`, 'Error reading log file', e);
          }
        }
        ...
        if (fs.existsSync(vulnFile)) {
          try {
            ...
            if (Array.isArray(vulns) && vulns.length > lastVulnCount) {
              ...
              log.info(`SSE /api/scans/${id}/stream`, `${newVulns.length} new vulnerability(ies) found`, {
            ...
            log.error(`SSE /api/scans/${id}/stream`, 'Error reading vulnerabilities.json', e);
          }
        }
        ...
        if (fs.existsSync(runFile)) {
          try {
            const run = JSON.parse(fs.readFileSync(runFile, 'utf-8'));
            if (run.status !== 'running') {
              log.info(`SSE /api/scans/${id}/stream`, `Scan finished (status=${run.status}), closing SSE stream`, { exitCode: run.exitCode });
              controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'status', status: run.status })}\n\n`));
              clearInterval(watchInterval);
              controller.close();
            }
          } catch (e) {
            log.error(`SSE /api/scans/${id}/stream`, 'Error reading run.json in watch loop', e);
          }
        }
      }, 500);

      return () => {
        log.info(`SSE /api/scans/${id}/stream`, 'Client disconnected, clearing watch interval');
        clearInterval(watchInterval);
      };
    },
  });
