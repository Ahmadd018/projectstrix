...
Examples:
  ...
  strix --target https://app.com --instruction-file /path/to/detailed_instructions.md
    ...
    parser.add_argument(
        "--false-positive-file",
        type=str,
        help="Path to a markdown file listing known false positives for the in-scope targets. "
        "Its contents are placed in the sandbox so agents can consult it before reporting a "
        "finding and discard confirmed false positives. Kept separate from --instruction so the "
        "bulk detail never enters the prompt context (e.g., '--false-positive-file ./fps.md').",
    )
    ...
    # Known false positives are read here but kept apart from args.instruction: the
    # bulk detail is materialized into the sandbox (not the prompt) so agents pull
    # it on demand rather than carrying it in context.
    args.false_positives = ""
    if getattr(args, "false_positive_file", None):
        fp_path = Path(args.false_positive_file)
        try:
            with fp_path.open(encoding="utf-8") as f:
                args.false_positives = f.read().strip()
        except Exception as e:
            parser.error(f"Failed to read false-positive file '{fp_path}': {e}")
