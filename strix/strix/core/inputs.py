...
# Path, inside the sandbox, where the known-false-positive registry is written
# when the scan is started with false positives. Agents read it on demand; the
# bulk detail deliberately never enters the prompt context.
FALSE_POSITIVE_REGISTRY_PATH = "/workspace/.strix/false_positives.md"
...
def build_scope_context(scan_config: dict[str, Any]) -> dict[str, Any]:
    ...
    # When the scan carries a known-false-positive registry, expose only its path
    # (not its contents) so every agent's base prompt can point to it. Both the
    # root and child agents receive this context, so the directive applies tree-wide.
    if (scan_config.get("false_positives") or "").strip():
        context["false_positive_registry_path"] = FALSE_POSITIVE_REGISTRY_PATH
