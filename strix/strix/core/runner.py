...
async def run_strix_scan(
    *,
    ...
    # Materialize the known-false-positive registry into the sandbox so agents can
    # read it on demand (the base prompt points here). Contents stay out of the
    # prompt context; only the path is surfaced via scope_context.
    false_positives = str(scan_config.get("false_positives") or "").strip()
    if false_positives:
        try:
            await sandbox_session.write(
                Path(FALSE_POSITIVE_REGISTRY_PATH),
                io.BytesIO(false_positives.encode("utf-8")),
            )
            logger.info(
                "Wrote false-positive registry to sandbox at %s (%d bytes)",
                FALSE_POSITIVE_REGISTRY_PATH,
                len(false_positives),
            )
        except Exception:
            logger.exception("failed to write false-positive registry to sandbox")
